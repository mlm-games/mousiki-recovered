...
const RES_SCALE: f32 = 32_768.0;
const RES_SCALE_24: f32 = RES_SCALE * 256.0;
...
impl<'a> MappingMatrixView<'a> {
    pub fn cell(&self, row: usize, col: usize) -> i16 {
        let idx = matrix_index(self.rows, row, col);
        self.data[idx]
    }
...
pub fn mapping_matrix_get_size(rows: usize, cols: usize) -> Option<usize> {
    if rows > MAX_CHANNELS || cols > MAX_CHANNELS {
        return None;
    }
...
pub fn mapping_matrix_multiply_channel_in_float(
    ...
    for frame in 0..frame_size {
        ...
        for col in 0..input_rows {
            let coeff = f32::from(matrix.cell(output_row, col));
...
pub fn mapping_matrix_multiply_channel_out_short(
    ...
    for frame in 0..frame_size {
        ...
        for row in 0..output_rows {
            ...
            let idx = matrix_index(output_rows, row, frame);
            let updated = i32::from(output[idx]).wrapping_add(delta);
            output[idx] = updated as i16;
        }
    }
}
...
pub fn mapping_matrix_multiply_channel_in_int24(
    ...
    for frame in 0..frame_size {
        let mut tmp = 0.0f64;
        for col in 0..input_rows {
            let coeff = f64::from(matrix.cell(output_row, col));
...
pub fn mapping_matrix_multiply_channel_out_int24(
    ...
    for frame in 0..frame_size {
        ...
        for row in 0..output_rows {
            ...
            let delta = ((tmp + 16_384) >> 15) as i32;
            let idx = matrix_index(output_rows, row, frame);
            output[idx] = output[idx].wrapping_add(delta);
        }
    }
}

#[inline]
fn res_to_int16(value: OpusRes) -> i16 {
    float2int16(value)
}

#[inline]
fn res_to_int24(value: OpusRes) -> i32 {
    float2int(value * RES_SCALE_24)
}

#[cfg(test)]
#[inline]
fn int16_to_res(value: i16) -> OpusRes {
    f32::from(value) / RES_SCALE
}
...
mod tests {
    ...
    fn assert_res_matches_i16(actual: &[OpusRes], expected: &[i16]) {
        ...
        for (i, (&res, &want)) in actual.iter().zip(expected.iter()).enumerate() {
            let converted = res_to_int16(res);
            assert!(
                (i16::from(converted) - want).abs() <= 1,
                "mismatch at {i}: {converted} vs {want}"
            );
        }
    }

    fn assert_i16_matches(actual: &[i16], expected: &[i16]) {
        assert_eq!(actual.len(), expected.len());
        for (i, (&got, &want)) in actual.iter().zip(expected.iter()).enumerate() {
            assert!((got - want).abs() <= 1, "mismatch at {i}: {got} vs {want}");
        }
    }
    ...
    fn simple_matrix_operations_follow_reference() {
        ...
        // _in_short
        let mut output_res = vec![0.0; SIMPLE_MATRIX_EXPECTED.len()];
        ...
        // _out_short
        let mut output_int16 = vec![0i16; SIMPLE_MATRIX_EXPECTED.len()];
        ...
        assert_i16_matches(&output_int16, &SIMPLE_MATRIX_EXPECTED);
